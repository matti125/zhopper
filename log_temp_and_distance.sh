...
export TAG=${TAG:-"printer=mazas-2,housing=plenum,flow_direction=stock"} 
...
./log_dti_distance.sh &     # Start log_temperatures.sh in the background
DTI_PID=$!          # Store the PID of the process

# Save the PIDs to a file (optional)
echo "$BEACON_PID" > ./pids.txt
echo "$TEMPERATURE_PID" >> ./pids.txt
echo "$DTI_PID" >> ./pids.txt
